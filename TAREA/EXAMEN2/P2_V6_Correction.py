...
def crear_paredes_pymunk(space):
    ...
    for s in [suelo, pared_izquierda, pared_derecha, techo_caida]:
        ...
        s.friction = 1.0
        if s != techo_caida:
             # Tipo 0 para las paredes y suelo
             s.collision_type = 0
        space.add(s)
...
def fusion_callback(arbiter, space, data):
    ...
    if fruta_a.radio == fruta_b.radio and not fruta_a.fusionada and not fruta_b.fusionada:
        ...
        # Añadir a la lista de eliminación (se procesará después del 'step')
        ...
        # Comprobar si es la fruta final (Calabaza)
        if fruta_a.radio == radios_frutas[-1]:
            print("¡¡FUSIÓN DE CALABAZAS!! ¡HAS GANADO!")
            JUEGO_GANADO = True
            # No se crea una nueva fruta, ya que nuevo_radio == fruta_a.radio
        
        # Si el nuevo radio es diferente, crear la siguiente fruta
        if nuevo_radio != fruta_a.radio: 
            nueva_fruta = Fruta(nuevo_radio, pos_nueva, space)
            data['nuevas_frutas_en_step'].append(nueva_fruta)
            data['frutas'].append(nueva_fruta) # Añadir a la lista principal
    ...
    try:
        ...
        for radio, nombre, img_path in FRUTAS_INFO:
            try:
                ...
                print(e)
                # Crear una superficie de fallback (un círculo rosa)
...
# --- NUEVO: Delay para que aparezca la siguiente fruta ---
...
for radio in radios_frutas:
    ...
    handler.data["frutas"] = frutas # Referencia a la lista principal de frutas
...
def obtener_nueva_fruta_aleatoria():
    # Elige una de las primeras 3 frutas (cereza, fresa, uva)
    radio_inicial = random.choice(radios_frutas[:3])
    # Posición inicial (arriba, en la zona de caída)
...
JUEGO_GANADO = False
JUEGO_PERDIDO = False
# Línea de "Game Over" (en coordenadas Pymunk, desde abajo)
# Un poco por debajo del techo (ALTO_JUEGO_CAJA)
...
while running:
    ...
    if not ret:
        ...
        break
    frame_cv2 = cv2.flip(frame_cv2, 1) # Voltear horizontalmente
    ...
    todos_dedos_extendidos = False # Para iniciar/reiniciar
    
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            # Dibujar los landmarks en el frame de CV2
            mp_draw.draw_landmarks(frame_cv2, hand_landmarks, mp_hands.HAND_CONNECTIONS)
            
            # 1. Control de Posición (Punta del dedo índice)
            ...
            # 2. Control de Agarre/Suelte (Distancia pulgar-índice)
            ...
            # 3. Gesto de Inicio (Palma abierta)
            dedos_extendidos = []
            dedos_indices = [
            ...
            # Comprobar pulgar (un poco más simple)
            thumb_is_extended = False
            thumb_tip_lm = hand_landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP]
            thumb_ip_lm = hand_landmarks.landmark[mp_hands.HandLandmark.THUMB_IP] # Articulación intermedia
            # Heurística simple para el pulgar "abierto"
    ...
    # Limitar la posición X para que la fruta no salga de la caja
    if fruta_cayendo:
    ...
    if ESTADO_ACTUAL == MENU:
        if todos_dedos_extendidos: # Gesto para empezar
            ESTADO_ACTUAL = JUGANDO
            
            # --- Resetear estados de victoria/derrota y el juego ---
            JUEGO_GANADO = False
            JUEGO_PERDIDO = False
            tiempo_en_zona_peligro_inicio = 0
            
            # Limpiar frutas anteriores del 'space' y de la lista
            for f in frutas:
                f.remover_de_space()
            frutas = []
            
            # --- OPTIMIZACIÓN: Iterar sobre la lista guardada para resetear data ---
            for handler in collision_handlers:
                # Simplemente reseteamos el 'data' del handler existente
                handler.data["frutas"] = frutas # 'frutas' ahora está vacía
                handler.data["nuevas_frutas_en_step"] = [] 
                handler.data["frutas_a_eliminar_en_step"] = [] 
            ...
            fruta_cayendo = None
            tiempo_ultimo_lanzamiento = 0
            # --- NUEVO: Resetear el tiempo de soltar ---
            tiempo_ultimo_soltar_real = 0 
            
    elif ESTADO_ACTUAL == JUGANDO:
        
        # --- NUEVO: Obtener el tiempo una vez por frame ---
        tiempo_actual_bucle = pygame.time.get_ticks() / 1000.0
        
        # 1. Generar nueva fruta si no hay una Y ha pasado el delay
        if not fruta_cayendo:
            # --- MODIFICADO: Lógica de Delay ---
            # Solo crear una nueva si ha pasado el tiempo de delay
            if (tiempo_actual_bucle - tiempo_ultimo_soltar_real) > DELAY_NUEVA_FRUTA:
                fruta_cayendo = obtener_nueva_fruta_aleatoria()
            # --- FIN MODIFICADO ---
        
        # 2. Mover y soltar la fruta que cae (si existe)
        if fruta_cayendo:
            if mano_cerrada_para_agarrar:
                # "Agarrar" la fruta: moverla y resetear el delay de lanzamiento
            ...
            if mano_abierta_para_soltar:
                if (tiempo_actual_bucle - tiempo_ultimo_lanzamiento) > DELAY_LANZAMIENTO:
                    ...
                    # --- NUEVO: Registrar el tiempo de "soltar" para el delay de la próxima fruta ---
                    tiempo_ultimo_soltar_real = tiempo_actual_bucle
                    # --- FIN DEL NUEVO CÓDIGO ---
                        
        # 4. Actualizar la simulación de Pymunk
        space.step(1.0/FPS)

        # 5. Procesar fusiones (eliminaciones y adiciones)
        frutas_a_eliminar_final = []
        frutas_a_añadir_final = []
        
        # --- OPTIMIZACIÓN: Iterar sobre la lista guardada, no buscar en el 'space' ---
        for handler in collision_handlers:
            ...
            # Limpiar las listas en el handler para el próximo frame
        ...
        # Eliminar las frutas fusionadas (usando 'set' para evitar duplicados)
        ...
        # Añadir las nuevas frutas creadas
        for nueva_fruta in frutas_a_añadir_final:
            nueva_fruta.agregar_a_space()
            # (Ya están en la lista 'frutas' porque se añaden en el callback)

        # --- NUEVO: Detectar si hubo fusiones en este frame ---
        hubo_fusiones_este_frame = len(frutas_a_añadir_final) > 0 or len(frutas_a_eliminar_final) > 0
        # --- FIN DEL NUEVO CÓDIGO ---
        
        # Resetear el estado 'fusionada' de todas las frutas
        for fruta in frutas:
            fruta.fusionada = False

        # 6. Lógica de Victoria/Derrota
        
        # 6.1. Comprobar Victoria (la flag se activa en el callback)
        if JUEGO_GANADO:
            ESTADO_ACTUAL = FINALIZADO

        # 6.2. Comprobar Derrota (línea superior)
        else: # Solo comprobar si no hemos ganado ya
            fruta_en_peligro = False
            for fruta in frutas:
                # Si el borde superior de la fruta (centro + radio)
                # supera la línea de perder
                if (fruta.cuerpo.position.y + fruta.radio) > LINEA_PERDER_Y:
                    fruta_en_peligro = True
                    break
            
            # Reutilizamos la variable de tiempo de arriba
            tiempo_actual = tiempo_actual_bucle
            
            if fruta_en_peligro:
                # --- MODIFICADO: Lógica de Game Over Justo ---
                # Solo contar si NO acaban de ocurrir fusiones
                if not hubo_fusiones_este_frame:
                    if tiempo_en_zona_peligro_inicio == 0:
                        ...
                        # Si ha pasado el tiempo, se pierde
                        print("¡GAME OVER! Fruta demasiado alta.")
                        JUEGO_PERDIDO = True
                        ESTADO_ACTUAL = FINALIZADO
                else:
                    # Si hubo fusiones, reseteamos el contador.
                    # Esto da tiempo a que las frutas se asienten.
                    tiempo_en_zona_peligro_inicio = 0
                # --- FIN DE LA MODIFICACIÓN ---
            else:
                # Si no hay fruta en peligro, resetear el contador
                tiempo_en_zona_peligro_inicio = 0

    elif ESTADO_ACTUAL == FINALIZADO:
        # El juego está pausado (no hay 'space.step()')
        # Comprobamos el gesto para reiniciar.
        if todos_dedos_extendidos:
            ESTADO_ACTUAL = MENU # Volvemos al menú


    # --- Dibujo en Pygame ---
    pantalla.fill((0, 0, 50)) # Color de fondo del panel de control
    
    # Dibujar el fondo del juego
    if FONDO_JUEGO:
        # El fondo se dibuja en la parte de abajo (desplazado por ALTO_PANEL_CONTROL)
        pantalla.blit(FONDO_JUEGO, (0, ALTO_PANEL_CONTROL))
    else:
        # Fondo de fallback si la imagen falló
        pygame.draw.rect(pantalla, (20, 20, 20), (0, ALTO_PANEL_CONTROL, ANCHO_JUEGO, ALTO_JUEGO_CAJA))


    # Dibujar la línea de "Game Over"
    if ESTADO_ACTUAL == JUGANDO:
        # Convertir Y de Pymunk (abajo=0) a Y de Pygame (arriba=0)
        y_pygame_linea = ALTO_JUEGO - LINEA_PERDER_Y 
        
        linea_color = (255, 0, 0) # Rojo
        
        # Hacer que la línea parpadee si hay peligro
        if tiempo_en_zona_peligro_inicio > 0:
            if int(pygame.time.get_ticks() / 250) % 2 == 0:
                 linea_color = (255, 150, 150) # Rojo claro
    ...
    # Dibujar las frutas (solo en estado JUGANDO o FINALIZADO)
    if ESTADO_ACTUAL == JUGANDO or ESTADO_ACTUAL == FINALIZADO:
        for fruta in frutas:
            fruta.dibujar(pantalla)
        if fruta_cayendo and ESTADO_ACTUAL == JUGANDO: # Solo dibujar la fruta que cae si estamos jugando
            fruta_cayendo.dibujar(pantalla)
            
        # Dibujar línea guía solo si estamos jugando y sosteniendo una fruta
