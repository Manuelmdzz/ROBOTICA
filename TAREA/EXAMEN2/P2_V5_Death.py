...
class Fruta:
    def __init__(self, radio, pos, space):
        ...
        if radio in IMAGENES_FRUTAS:
            ...
            self.imagen = IMAGENES_FRUTAS[20] 
...
def fusion_callback(arbiter, space, data):
    # --- NUEVO: Acceder a la variable global de victoria ---
    global JUEGO_GANADO
    ...
    if fruta_a.radio == fruta_b.radio and not fruta_a.fusionada and not fruta_b.fusionada:
        ...
        # --- NUEVO: Comprobar si es la fruta final (Calabaza) ---
        # radios_frutas[-1] es el radio de la calabaza (la más grande)
...
# --- CAMBIO: Nueva función para cargar y escalar imágenes (Sin cambios) ---
def cargar_recursos():
...
# --- NUEVO: Variables de Victoria/Derrota ---
JUEGO_GANADO = False
JUEGO_PERDIDO = False
...
while running:
    for evento in pygame.event.get():
        if evento.type == pygame.QUIT:
            running = False
        # ... (evento de K_SPACE para debug, sin cambios) ...
    ...
    if ESTADO_ACTUAL == MENU:
        if todos_dedos_extendidos:
            ...
            # --- NUEVO: Resetear estados de victoria/derrota y el juego ---
            JUEGO_GANADO = False
            JUEGO_PERDIDO = False
            tiempo_en_zona_peligro_inicio = 0
            
            # Limpiar frutas anteriores
            for f in frutas:
                f.remover_de_space()
            frutas = []
            
            # Reiniciar handlers (importante para limpiar listas de data)
            for radio in radios_frutas:
                # Quitar handler viejo si existe
                handler_viejo = space.add_collision_handler(radio, radio)
                
                # Añadir handler nuevo
                handler = space.add_collision_handler(radio, radio)
                if handler:
                    handler.data["frutas"] = frutas
                    handler.data["nuevas_frutas_en_step"] = [] 
        ...
        # 1. Comprobar Victoria (la flag se activa en el callback)
        if JUEGO_GANADO:
            ESTADO_ACTUAL = FINALIZADO

        # 2. Comprobar Derrota (línea superior)
    ...
    # --- NUEVO: Lógica del estado finalizado (para reiniciar) ---
    ...
    # --- NUEVO: Dibujar la línea de "Game Over" ---
    ...
    # --- NUEVO: Lógica de pantalla final ---
    elif ESTADO_ACTUAL == FINALIZADO:
        mensaje_final = ""
