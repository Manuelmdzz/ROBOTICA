...
ALTO_PANEL_CONTROL = 150 # Aumentamos el espacio en la parte superior para la cámara y mensajes

# Alto TOTAL de la ventana
ALTO_JUEGO = ALTO_JUEGO_CAJA + ALTO_PANEL_CONTROL 

# --- Configuración de Cámara para CV2 ---
ANCHO_CAMARA = 300 # Aumentado
ALTO_CAMARA = 120 # Aumentado
...
while running:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # Y que el pulgar también está extendido (su punta a la derecha/izquierda de su base)
            
            dedos_extendidos = []
            
            # Índice, Corazón, Anular, Meñique (Landmarks de puntas y bases)
            dedos_indices = [
            ...
            # Para el pulgar, su punta debe estar a la derecha de la base (para mano derecha, x mayor)
            # O a la izquierda (para mano izquierda, x menor). Asumiremos una dirección general
            # Simplemente comprobamos que no esté excesivamente doblado hacia adentro.
            ...
            # Si la punta del pulgar está más alejada del centro de la palma (WRIST) que el nudillo
            wrist_lm = hand_landmarks.landmark[mp_hands.HandLandmark.WRIST]
            dist_tip_wrist = ((thumb_tip_lm.x - wrist_lm.x)**2 + (thumb_tip_lm.y - wrist_lm.y)**2)**0.5
            dist_ip_wrist = ((thumb_ip_lm.x - wrist_lm.x)**2 + (thumb_ip_lm.y - wrist_lm.y)**2)**0.5
            if dist_tip_wrist > dist_ip_wrist: # Una aproximación simple
